...
namespace HeyTeam.Web.Areas.Administration.Controllers {
	...
	public class DashboardApiController : Controller
    {
		...
		{
			var events = eventQuery.GetUpcomingEvents(new UpcomingEventsRequest { MemberId = model.MemberId, ClubId = club.Guid, Membership = Membership.Administrator });
		...
		public IActionResult GetLatestFeedback(DashboardModel model) {
			var response = feedbackQuery.GetLatestFeedback(new LatestFeedbackRequest { ClubId = club.Guid, MemberId = model.MemberId, Membership = Membership.Administrator });
			return new JsonResult(response);
		}
        ...
        {
			var squads = squadQuery.GetSquadSummary(club.Guid);
